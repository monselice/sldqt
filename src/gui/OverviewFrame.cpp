...
void OverviewFrame::actualBalanceUpdated(quint64 _balance) {
	m_ui->WB_label->setText("Wallet Balance: "+CurrencyAdapter::instance().formatAmount(_balance)+" "+CurrencyAdapter::instance().getCurrencyTicker().toUpper());
...
void OverviewFrame::pendingBalanceUpdated(quint64 _balance) {
	if (_balance>0) {
		m_ui->LB_label->setVisible(true);
		m_ui->LB_label->setText("+ "+CurrencyAdapter::instance().formatAmount(_balance)+" "+CurrencyAdapter::instance().getCurrencyTicker().toUpper()+" locked");
	} else {
		m_ui->LB_label->setVisible(false);
		m_ui->LB_label->setText("");
	}
